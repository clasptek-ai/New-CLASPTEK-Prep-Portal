...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const res = await pool.query('SELECT * FROM modules WHERE id = $1 AND deleted_at IS NULL', [
      moduleId,
    ]);
    ...
    const compRes = await pool.query(
      'SELECT * FROM competencies WHERE module_id = $1 AND deleted_at IS NULL ORDER BY display_order ASC',
      [moduleId]
    );
