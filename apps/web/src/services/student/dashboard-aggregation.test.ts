...
describe('DashboardAggregationService Unit Tests', () => {
  test('getOverview aggregates student profile, course progress, assessment summary, and AI summary into DashboardOverviewDto', async () => {
    const overview = await DashboardAggregationService.getOverview('stud-active-123');
    expect(overview).toBeDefined();
    expect(overview.profile.studentName).toBe('Alex Mercer');
  ...
  test('getActivity returns paginated activity feed sorted newest first', async () => {
    const activity = await DashboardAggregationService.getActivity(1, 3);
  ...
  test('getCalendar returns scheduled academic events for Day, Week, and Month views', async () => {
    const monthCalendar = await DashboardAggregationService.getCalendar('MONTH');
    expect(monthCalendar.view).toBe('MONTH');
    expect(monthCalendar.events.length).toBeGreaterThan(0);

    const dayCalendar = await DashboardAggregationService.getCalendar('DAY');
  ...
  test('getAchievements calculates badges, milestones, XP, and streak without certificates', async () => {
    const achievements = await DashboardAggregationService.getAchievements();
