...
export const instructorDashboardService = {
  async getDashboardData(): Promise<DashboardAggregatedData> {
    try {
      return await apiClient.get<DashboardAggregatedData>(
        '/api/v1/instructor/dashboard/aggregated'
      );
    } catch {
        ...
        recentActivity: [
          {
            time: '5m ago',
            type: 'PRACTICE',
            msg: 'Jane Smith completed Grammar modifiers session (Score: 85%)',
          },
          {
            time: '1h ago',
            type: 'SUBMISSION',
            msg: 'John Doe submitted Advanced Essay Syntax assignment',
          },
          {
            time: '3h ago',
            type: 'MOCK',
            msg: 'Bob Johnson finished Diagnostic Mock Exam A (Score: 68%)',
          },
          {
            time: '1d ago',
            type: 'INTERVENTION',
            msg: 'AI Coach flagged student Alice Brown (Ready Score: 52%) as at-risk',
          },
        ],
        notifications: [
