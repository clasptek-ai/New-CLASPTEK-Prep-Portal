...
export const analyticsService = {
  async getStudentDashboard(studentId: string, profileId: string): Promise<StudentDashboardData> {
    try {
      ...
      return {
        ...
        predictionTrend: [80, 81, 82, 84.50],
  ...
  async getTrends(category: string): Promise<{ category: string; trendPoints: Array<{ date: string; value: number }> }> {
    try {
      return await apiClient.get<{ category: string; trendPoints: Array<{ date: string; value: number }> }>(
