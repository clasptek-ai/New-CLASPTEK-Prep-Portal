...
export function IntegrationCard({
  ...
  onConfigure: (p: ProviderDefinition) => void;
}) {
  return (
    <Card title={provider.name} actions={<Badge>{provider.category}</Badge>}>
      ...
      >
        <p style={{ margin: 0, color: '#cbd5e1' }}>
          Supports: {provider.supportsOAuth && 'OAuth2 '} {provider.supportsApiKeys && 'API-Keys '}{' '}
          {provider.supportsWebhooks && 'Webhooks'}
        </p>
        <div
          style={{
            display: 'flex',
        ...
        >
          <span style={{ fontSize: '0.75rem', color: '#64748b' }}>
            Release Status: **{provider.status.toUpperCase()}**
          </span>
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#cbd5e1' }}>
        {status}{' '}
        {latencyMs && <span style={{ color: '#64748b', fontWeight: 500 }}>({latencyMs}ms)</span>}
      </span>
    </div>
  );
}
          ...
          {provider.supportsApiKeys && (
            <div>
              ...
              <Input
                ...
                required
              />
              <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.65rem', color: '#64748b' }}>
                Secrets are masked and hidden after creation.
              </p>
            </div>
          )}
