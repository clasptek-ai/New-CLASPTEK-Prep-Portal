...
export default function StudentLoading() {
  ...
  return (
    ...
    >
      ...
      >
        {/* Logo */}
        <div aria-hidden="true" style={{ ...shimmer, height: 36, width: '80%', marginBottom: '1rem' }} />
        {/* Nav items */}
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} aria-hidden="true" style={{ ...shimmer, height: 32, width: i % 3 === 0 ? '90%' : '70%' }} />
        ))}
      </div>
      ...
      <div style={{ flex: 1, padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              aria-hidden="true"
              style={{ ...shimmer, height: 120, borderRadius: 12 }}
            />
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} aria-hidden="true" style={{ ...shimmer, height: 16, width: i === 2 ? '60%' : '100%' }} />
          ))}
        </div>
      </div>
