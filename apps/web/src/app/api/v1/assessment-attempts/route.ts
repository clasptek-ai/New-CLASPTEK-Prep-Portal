...
export async function POST(req: NextRequest) {
  try {
    ...
    const studentId = session?.userId || body.candidateId || (process.env.NODE_ENV === 'test' ? req.headers.get('x-student-id') : null);
    if (!studentId) {
    ...
    // 1. Resolve student target programme & active assigned assessment definition
    ...
    let definitionId = body.assessmentId;
    let definition: any = null;

    if (definitionId) {
      const defRes = await pool.query(`SELECT * FROM public.assessment_definitions WHERE id = $1`, [definitionId]);
    ...
    if (!definition) {
      return NextResponse.json(
        { success: false, error: 'NO_PUBLISHED_ASSESSMENT', message: 'No published assessment definition found.' },
        { status: 404 }
      );
    }
    ...
    // 4. Generate & Freeze Paper Snapshot with Version Metadata
    ...
    // 5. Create Assessment Attempt (status = 'IN_PROGRESS') & Log Event
    ...
    await pool.query(
      ...
      [attemptId, studentId, definition.id, now.toISOString(), expiresAt.toISOString(), durationMins, JSON.stringify(paperSnapshot), tenantId]
    );
    ...
    await pool.query(
      ...
      [attemptId, JSON.stringify({ assessmentId: definition.id, durationMinutes: durationMins })]
    );
    ...
    return NextResponse.json(
      {
        ...
        meta: { timestamp: now.toISOString(), version: 1 },
      },
      { status: 201 }
    );
  } catch (err: any) {
    console.error('POST /api/v1/assessment-attempts error:', err);
