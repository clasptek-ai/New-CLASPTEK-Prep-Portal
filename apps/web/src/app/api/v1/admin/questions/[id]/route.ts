  ...
  try {
    ...
    if (action === 'addAnswerOption') {
      if (!verNoVo || !optId || !optCode || !optText || displayOrder === undefined) {
        return NextResponse.json(
          { code: 'VALIDATION_ERROR', message: 'Missing option parameters' },
          { status: 400 }
        );
      }
      ...
      if (!verNoVo || !solId || !explanation) {
        return NextResponse.json(
          { code: 'VALIDATION_ERROR', message: 'Missing solution parameters' },
          { status: 400 }
        );
      }
      ...
      if (!verNoVo || !rubId || !criteria || maxPoints === undefined) {
        return NextResponse.json(
          { code: 'VALIDATION_ERROR', message: 'Missing rubric parameters' },
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        { code: 'VALIDATION_ERROR', message: 'Invalid action parameter' },
        { status: 400 }
      );
    }
    ...
    logger.error(
      'PATCH /api/v1/admin/questions/[id] failure',
