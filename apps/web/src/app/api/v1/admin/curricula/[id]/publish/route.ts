...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { publishCurriculumVersionHandler, logger } = await getCurriculumContext();
  try {
    ...
    if (!versionId || expectedVersion === undefined) {
      return NextResponse.json(
        { code: 'VALIDATION_ERROR', message: 'Missing versionId or expectedVersion' },
        { status: 400 }
      );
    }
    ...
    logger.error(
      'POST /api/v1/admin/curricula/[id]/publish failure',
