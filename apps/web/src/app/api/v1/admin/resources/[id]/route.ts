  ...
  try {
    ...
    if (action === 'createVersion') {
      ...
      if (!versionNo || !key || value === undefined) {
        return NextResponse.json(
          { code: 'VALIDATION_ERROR', message: 'Missing metadata parameters' },
          { status: 400 }
        );
      }
      const versionRepo = new (require('@clasptek/persistence').PostgresResourceVersionRepository)(
      ...
      (resource as any).update(
        name || (resource as any).title || (resource as any).name,
        description !== undefined ? description : (resource as any).description
      );
    ...
    logger.error(
      'PATCH /api/v1/admin/resources/[id] failure',
