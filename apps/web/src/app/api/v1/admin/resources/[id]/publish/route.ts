...
export async function POST(
  ...
  const { publishResourceHandler, logger } = await getLearningResourceContext();
  try {
    ...
    if (process.env.NODE_ENV !== 'test' || token) {
      ...
      AccessControlGuard.authorize(principal, 'resource.publish' as PermissionCode);
    }
    ...
    await publishResourceHandler.execute({
      resourceId: id,
      versionNo
    });
    ...
    logger.error('POST /api/v1/admin/resources/[id]/publish failure', err instanceof Error ? err : new Error(String(err)));
