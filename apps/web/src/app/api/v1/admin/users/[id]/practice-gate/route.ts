...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const logger = new ConsoleLogger('PracticeGateRoute');
    ...
    if (locked === undefined) {
      const currentRes = await pool.query(
        'SELECT practice_gate_locked FROM public.users WHERE id = $1',
        [userId]
      );
      if (currentRes.rows.length > 0) {
        newLockedState = !currentRes.rows[0].practice_gate_locked;
      }
    }
    ...
    // Update public.users practice gate state
    await pool.query(
      `UPDATE public.users
       SET practice_gate_locked = $1,
           practice_gate_reason = $2,
           practice_gate_updated_at = NOW(),
           practice_gate_updated_by = $3,
           updated_at = NOW()
       WHERE id = $4`,
    ...
    await pool
      .query(
       ...
       VALUES (gen_random_uuid(), $1, 'ADMIN_TOGGLED_PRACTICE_GATE', 'public.users', $2, $3, NOW())`,
        [session.userId, userId, JSON.stringify({ practice_gate_locked: newLockedState, reason })]
      )
      .catch(() => null);

    return NextResponse.json({
      success: true,
      practiceUnlocked: !newLockedState,
      message: `Practice gate access ${newLockedState ? 'locked' : 'unlocked'} successfully.`,
    });
  } catch (err: unknown) {
    return NextResponse.json(
