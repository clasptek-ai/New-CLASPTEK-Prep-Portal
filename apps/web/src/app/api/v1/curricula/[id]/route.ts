...
export async function GET(
  ...
  try {
    ...
    const versions = await Promise.all(cur.versions.map(async (v: any) => {
    ...
    logger.error('GET /api/v1/curricula/[id] failure', err instanceof Error ? err : new Error(String(err)));
    return NextResponse.json({ code: 'NOT_FOUND', message: 'Curriculum not found' }, { status: 404 });
  }
}
