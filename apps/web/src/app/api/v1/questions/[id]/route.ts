...
export async function GET(req: NextRequest, _params: { params: Promise<{ id: string }> }) {
  ...
  const question: any = await getQuestionHandler.execute(id);
  if (!question) {
  ...
  return NextResponse.json({
    ...
    versions: question.versions.map((v: any) => ({
      id: v.id,
      ...
      answerOptions: v.answerOptions.map((opt: any) => ({
        id: opt.id,
        code: opt.code,
      ...
      mediaAssets: v.mediaAssets.map((med: any) => ({
        id: med.id,
    ...
    ownership: question.ownership
      ...
      : null,
    dependencies: question.dependencies.map((d: any) => ({
