...
export async function GET(req: NextRequest, { params }: { params: Promise<{ examType: string }> }) {
  try {
    ...
    if (sections.length === 0) {
      if (examType === 'English Proficiency') {
        sections = [
          {
            id: 'sec-g',
            code: 'ENG-PROF-GRAMMAR',
            examType,
            name: 'Grammar & Structure',
            displayOrder: 1,
            timeLimitMinutes: 10,
            instructions: 'Answer all grammar questions.',
          },
          {
            id: 'sec-r',
            code: 'ENG-PROF-READING',
            examType,
            name: 'Reading Comprehension',
            displayOrder: 2,
            timeLimitMinutes: 10,
            instructions: 'Read passage and answer.',
          },
          {
            id: 'sec-w',
            code: 'ENG-PROF-WRITING',
            examType,
            name: 'Writing Expression',
            displayOrder: 3,
            timeLimitMinutes: 10,
            instructions: 'Compose written essay.',
          },
          {
            id: 'sec-l',
            code: 'ENG-PROF-LISTENING',
            examType,
            name: 'Listening Comprehension',
            displayOrder: 4,
            timeLimitMinutes: 10,
            instructions: 'Listen to track and answer.',
          },
          {
            id: 'sec-s',
            code: 'ENG-PROF-SPEAKING',
            examType,
            name: 'Speaking & Oral Delivery',
            displayOrder: 5,
            timeLimitMinutes: 10,
            instructions: 'Record cue card response.',
          },
        ];
      } else {
        sections = [
          {
            id: 'sec-gen',
            code: `${examType.toUpperCase().replace(/\s+/g, '-')}-READINESS`,
            examType,
            name: `${examType} Readiness Section`,
            displayOrder: 1,
            timeLimitMinutes: 30,
            instructions: 'Answer readiness assessment items.',
          },
        ];
      }
    }
    ...
    const [passagesRes, audioRes, writingRes, speakingRes] = await Promise.all([
      pool
        .query('SELECT * FROM public.reading_passages WHERE exam_type = $1 LIMIT 1', [examType])
        .catch(() => ({ rows: [] })),
      pool
        .query('SELECT * FROM public.listening_tracks WHERE exam_type = $1 LIMIT 1', [examType])
        .catch(() => ({ rows: [] })),
      pool
        .query('SELECT * FROM public.writing_tasks WHERE exam_type = $1 LIMIT 1', [examType])
        .catch(() => ({ rows: [] })),
      pool
        .query('SELECT * FROM public.speaking_tasks WHERE exam_type = $1 LIMIT 1', [examType])
        .catch(() => ({ rows: [] })),
    ]);
    ...
    const totalDurationMinutes = sections.reduce(
      (acc: number, s: any) => acc + (s.timeLimitMinutes || 0),
      0
    );
    ...
    return NextResponse.json({
      ...
      contentAssets: {
        readingPassage: readingPassage
          ? { id: readingPassage.id, title: readingPassage.title, content: readingPassage.content }
          : null,
        listeningTrack: listeningTrack
          ? { id: listeningTrack.id, title: listeningTrack.title, url: listeningTrack.url }
          : null,
