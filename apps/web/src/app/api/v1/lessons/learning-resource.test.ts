...
vi.mock('pg', () => {
  const queryMock = vi.fn().mockImplementation(async (sql: string, params?: any[]) => {
    ...
    if (sql.includes('INSERT INTO public.lessons') || sql.includes('INSERT INTO lessons') || sql.includes('UPDATE public.lessons') || sql.includes('UPDATE lessons')) {
      if (params) {
        if (params.length === 16) {
          const [id, learning_module_id, code, slug, title, summary, _type, default_sequence_no, _est, _min, _max, _method, _policy, _req, status, lock] = params;
          mockLessons.set(id, { id, module_id: learning_module_id, code, slug, title, summary, name: title, description: summary, display_order: default_sequence_no, status, lock_version: lock });
        } else if (params.length === 15) {
          const [code, slug, title, summary, _type, default_sequence_no, _est, _min, _max, _method, _policy, _req, status, lock, id] = params;
          mockLessons.set(id, { id, code, slug, title, summary, name: title, description: summary, display_order: default_sequence_no, status, lock_version: lock });
    ...
    if (sql.includes('INSERT INTO public.lesson_versions') || sql.includes('INSERT INTO lesson_versions')) {
      if (params) {
    ...
    if (sql.includes('INSERT INTO public.content_blocks') || sql.includes('INSERT INTO content_blocks')) {
      if (params) {
    ...
    if (sql.includes('INSERT INTO public.learning_resources') || sql.includes('INSERT INTO learning_resources')) {
      if (params) {
        if (params.length >= 13) {
          const [id, code, slug, canonical_title, canonical_description, resource_type_id, _cat, _sens, _vis, _owner, _lang, _variant, status] = params;
          mockResources.set(id, { id, code, slug, canonical_title, canonical_description, name: canonical_title, description: canonical_description, resource_type: resource_type_id, status, lock_version: 0 });
        } else {
    ...
    if (sql.includes('INSERT INTO public.resource_variants') || sql.includes('INSERT INTO resource_variants')) {
      if (params) {
        const [id, learning_resource_id, code, language_code, region_code, accessibility_profile, variant_purpose, is_default, current_published_version_id, current_version_no, status] = params;
        mockResourceVariants.set(id, { id, learning_resource_id, code, language_code, region_code, accessibility_profile, variant_purpose, is_default, current_published_version_id, current_version_no, status });
      }
      return { rowCount: 1 };
    }
    if (sql.includes('INSERT INTO public.resource_versions') || sql.includes('INSERT INTO resource_versions') || sql.includes('INSERT INTO learning_resource_versions')) {
      if (params) {
        const [id, resource_variant_id, version_no, status, title, description] = params;
        mockResourceVersions.set(id, { id, resource_variant_id, version_no, status, name: title, description, lock_version: 0 });
      }
      return { rowCount: 1 };
    }
    ...
    if ((sql.includes('public.resource_metadata_definitions') || sql.includes('resource_metadata_definitions')) && !sql.includes('resource_metadata m')) {
      if (sql.includes('SELECT')) {
        const [namespace, metadata_key] = params || [];
        const def = Array.from(mockMetadataDefinitions.values()).find((d: any) => d.namespace === namespace && d.metadata_key === metadata_key);
    ...
    if (sql.includes('INSERT INTO public.resource_metadata') || sql.includes('INSERT INTO resource_metadata')) {
      if (params) {
        const [resource_version_id, metadata_definition_id, metadata_value_json] = params;
        mockMetadata.set(`${resource_version_id}_${metadata_definition_id}`, { resource_version_id, metadata_definition_id, metadata_value_json });
      }
      return { rowCount: 1 };
    }
    if (sql.includes('INSERT INTO public.storage_objects') || sql.includes('INSERT INTO storage_objects')) {
      if (params) {
        const [id, storage_provider, bucket_name, object_path, original_filename, detected_mime_type, size_bytes, etag] = params;
        mockStorageObjects.set(id, { id, storage_provider, bucket_name, object_path, original_filename, detected_mime_type, size_bytes, etag });
      }
      return { rowCount: 1 };
    }
    if (sql.includes('INSERT INTO public.resource_version_objects') || sql.includes('INSERT INTO resource_version_objects')) {
      if (params) {
        const [resource_version_id, storage_object_id, object_role] = params;
        mockResourceVersionObjects.set(`${resource_version_id}_${storage_object_id}`, { resource_version_id, storage_object_id, object_role });
      }
      return { rowCount: 1 };
    }
    ...
    if (sql.includes('SELECT * FROM public.lesson_versions') || sql.includes('SELECT * FROM lesson_versions')) {
      const id = params ? params[0] : '';
    ...
    if (sql.includes('SELECT * FROM public.content_blocks') || sql.includes('SELECT * FROM content_blocks')) {
      const id = params ? params[0] : '';
    ...
    if (sql.includes('SELECT * FROM public.learning_resources') || sql.includes('SELECT * FROM learning_resources')) {
      const id = params ? params[0] : '';
      ...
      if (resource) {
        return {
          rows: [{
            ...
            canonical_description: resource.description || resource.canonical_description || 'Desc',
    ...
    if (sql.includes('SELECT * FROM public.resource_versions') || sql.includes('SELECT * FROM resource_versions') || sql.includes('SELECT * FROM learning_resource_versions')) {
      const filterVal = params ? params[0] : '';
      const versions = Array.from(mockResourceVersions.values()).filter((v: any) => {
        if (Array.isArray(filterVal)) {
          return filterVal.includes(v.resource_variant_id) || filterVal.includes(v.learning_resource_id);
        }
    ...
    if (sql.includes('resource_version_objects') || sql.includes('storage_objects')) {
      ...
      const rvos = Array.from(mockResourceVersionObjects.values()).filter((r: any) => r.resource_version_id === versionId);
    ...
    if (sql.includes('SELECT * FROM public.resource_variants') || sql.includes('SELECT * FROM resource_variants')) {
      const lrId = params ? params[0] : '';
      const variants = Array.from(mockResourceVariants.values()).filter((v: any) => v.learning_resource_id === lrId);
    ...
    if (sql.includes('SELECT md.namespace') || sql.includes('resource_metadata m') || sql.includes('SELECT * FROM resource_metadata')) {
      const id = params ? params[0] : '';
      const rows: any[] = [];
