...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!parseRes.success) {
      return NextResponse.json({ code: 'VALIDATION_ERROR', message: parseRes.error.message }, { status: 400 });
    }

    const commandResult = await createExamProductHandler.execute(parseRes.data);
    if (commandResult.isFailure) {
      return NextResponse.json({ code: 'DOMAIN_ERROR', message: commandResult.error.message }, { status: 400 });
    }
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (queryResult.isFailure) {
      return NextResponse.json({ code: 'QUERY_ERROR', message: queryResult.error.message }, { status: 400 });
    }
