...
export default function AdminLoading() {
  ...
  return (
    ...
    >
      ...
      >
        <div aria-hidden="true" style={{ ...shimmer, height: 36, width: '75%', marginBottom: '1.25rem' }} />
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} aria-hidden="true" style={{ ...shimmer, height: 30, width: i % 4 === 0 ? '50%' : '85%' }} />
        ))}
      </div>
      ...
      <div style={{ flex: 1, padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div aria-hidden="true" style={{ ...shimmer, height: 36, width: '15%', borderRadius: 8 }} />
        </div>
          ...
          {Array.from({ length: 6 }).map((_, row) => (
            ...
            >
              {Array.from({ length: 5 }).map((_, col) => (
                <div key={col} style={{ ...shimmer, height: 14, width: col === 4 ? '60%' : '90%', borderRadius: 4 }} />
              ))}
            </div>
          ))}
        </div>
