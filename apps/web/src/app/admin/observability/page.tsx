        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
          ...
          >
            ...
            <Card title="System Status">
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#34d399', marginBottom: '0.5rem' }}>
                {metrics.system.status}
              </div>
              <div style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                DB Latency: <strong style={{ color: '#f8fafc' }}>{metrics.system.dbLatencyMs}ms</strong>
              </div>
              <div style={{ fontSize: '0.85rem', color: '#94a3b8', marginTop: '0.25rem' }}>
                Database: <strong style={{ color: '#f8fafc' }}>{metrics.system.databaseName}</strong>
              </div>
            </Card>

            {/* Candidates */}
            <Card title="Candidate Registrations">
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#60a5fa', marginBottom: '0.5rem' }}>
                {metrics.users.total} Total
              </div>
              <div style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                New (Last 24h): <strong style={{ color: '#34d399' }}>+{metrics.users.newLast24h}</strong>
              </div>
            </Card>

            {/* Assessments */}
            <Card title="Assessment Lifecycle">
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#a78bfa', marginBottom: '0.5rem' }}>
                {metrics.assessments.submitted} Submitted
              </div>
              <div style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                In-Progress: <strong style={{ color: '#fbbf24' }}>{metrics.assessments.inProgress}</strong> | Last 24h: <strong style={{ color: '#f8fafc' }}>{metrics.assessments.last24h}</strong>
              </div>
            </Card>

            {/* Content Stratification */}
            <Card title="Question Bank Stratification">
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#f472b6', marginBottom: '0.5rem' }}>
                {metrics.content.totalQuestions} Questions
              </div>
              <div style={{ fontSize: '0.8rem', color: '#94a3b8' }}>
                Found: <strong style={{ color: '#60a5fa' }}>{metrics.content.stratification.foundation}</strong> | Inter: <strong style={{ color: '#a78bfa' }}>{metrics.content.stratification.intermediate}</strong> | Adv: <strong style={{ color: '#f472b6' }}>{metrics.content.stratification.advanced}</strong>
              </div>
            </Card>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
            <Card title="Assessment Performance Overview">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  <span style={{ fontSize: '0.9rem', color: '#94a3b8' }}>Average Candidate Score:</span>
            ...
            <Card title="Observability Telemetry">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  ...
                  <strong style={{ color: '#f8fafc' }}>{metrics.telemetry.totalEventsLogged}</strong>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  ...
                  <span style={{ color: '#34d399', fontWeight: 600 }}>{metrics.system.dbLatencyMs} ms</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  <span style={{ color: '#94a3b8' }}>Sentry Status:</span>
                  <span style={{ color: '#34d399', fontWeight: 600 }}>ACTIVE (structured logs)</span>
                </div>
              </div>
            </Card>
          </div>
                  ...
                  <tbody>
                    {metrics.telemetry.recentErrors.map((err: any, idx: number) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #0f1729' }}>
                        ...
                        <td style={{ padding: '0.75rem 0.5rem', fontFamily: 'monospace', color: '#94a3b8' }}>
