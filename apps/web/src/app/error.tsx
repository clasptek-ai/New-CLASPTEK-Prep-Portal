'use client';

import React from 'react';

/**
 * Root Error Boundary — `apps/web/src/app/error.tsx`
 *
 * Next.js catches all rendering errors below the root layout and
 * renders this component. In production, raw error messages are
 * never shown to users. A correlation digest is shown for support.
 */
export default function RootError({
  error,
  reset,
}: {
  error: Error & { digest?: string };
  reset: () => void;
}) {
  const isProduction = process.env.NODE_ENV === 'production';

  return (
    <div
      style={{
        minHeight: '100vh',
        backgroundColor: '#090d16',
        color: '#f8fafc',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        padding: '2rem',
        fontFamily: 'Inter, system-ui, sans-serif',
      }}
    >
      <div
        role="alert"
        aria-live="assertive"
        style={{
          maxWidth: '520px',
          width: '100%',
          backgroundColor: '#0f1729',
          border: '1px solid #1e293b',
          borderRadius: '16px',
          padding: '2.5rem 2rem',
          textAlign: 'center',
          boxShadow: '0 25px 50px rgba(0,0,0,0.6)',
        }}
      >
        {/* Icon */}
        <div
          aria-hidden="true"
          style={{
            width: '56px',
            height: '56px',
            borderRadius: '50%',
            backgroundColor: 'rgba(239,68,68,0.12)',
            border: '1px solid rgba(239,68,68,0.3)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            margin: '0 auto 1.5rem',
            fontSize: '1.5rem',
          }}
        >
          ⚠️
        </div>

        <h1
          style={{
            margin: '0 0 0.75rem',
            fontSize: '1.4rem',
            fontWeight: 700,
            color: '#f8fafc',
          }}
        >
          Something went wrong
        </h1>

        <p
          style={{
            color: '#94a3b8',
            fontSize: '0.9rem',
            lineHeight: 1.6,
            margin: '0 0 1.75rem',
          }}
        >
          {isProduction
            ? 'An unexpected error occurred. Our team has been notified. Please try again or return home.'
            : (error?.message ?? 'An unexpected error occurred.')}
        </p>

        {/* Support reference */}
        {error?.digest && (
          <p
            style={{
              color: '#475569',
              fontSize: '0.75rem',
              marginBottom: '1.75rem',
              fontFamily: 'monospace',
            }}
          >
            Reference: {error.digest}
          </p>
        )}

        {/* Actions */}
        <div
          style={{
            display: 'flex',
            gap: '0.75rem',
            justifyContent: 'center',
            flexWrap: 'wrap',
          }}
        >
          <button
            id="error-try-again-btn"
            onClick={() => reset()}
            aria-label="Try loading this page again"
            style={{
              backgroundColor: '#2563eb',
              color: '#fff',
              border: 'none',
              borderRadius: '8px',
              padding: '0.65rem 1.25rem',
              fontSize: '0.875rem',
              fontWeight: 600,
              cursor: 'pointer',
              transition: 'background-color 0.15s',
            }}
          >
            Try Again
          </button>
          <a
            href="/"
            id="error-go-home-link"
            aria-label="Return to the Clasptek home page"
            style={{
              backgroundColor: 'transparent',
              color: '#cbd5e1',
              border: '1px solid #334155',
              borderRadius: '8px',
              padding: '0.65rem 1.25rem',
              fontSize: '0.875rem',
              fontWeight: 600,
              textDecoration: 'none',
              display: 'inline-block',
            }}
          >
            Return Home
          </a>
        </div>
      </div>
    </div>
  );
}
