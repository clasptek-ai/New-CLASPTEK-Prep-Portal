...
import { authoringCurriculumService, AuthoringProgramme } from '../../../services/authoring/curriculum.service';
...
export function ProgrammesScreen({ programmeId }: { programmeId?: string }) {
  ...
  const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'CURRICULUM' | 'OUTCOMES' | 'VERSIONS' | 'PUBLISHING'>('OVERVIEW');
  ...
  const columns = [
    {
      ...
      cell: (info: any) => (
        <span style={{ fontWeight: 600, color: '#60a5fa', cursor: 'pointer' }} onClick={() => router.push(`/authoring/programmes/${info.row.id}`)}>
          {info.row.name}
        </span>
      ),
      sortable: true
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Code: {selectedProg.code} | Version: v{selectedProg.version}</p>
          </div>
          <Button variant="secondary" onClick={() => router.push('/authoring/programmes')}>Back to Programmes List</Button>
        </div>

        {/* Tab navigators */}
        <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid #1e293b', paddingBottom: '0.5rem' }}>
          {(['OVERVIEW', 'CURRICULUM', 'OUTCOMES', 'VERSIONS', 'PUBLISHING'] as const).map(tab => (
            <button
              key={tab}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '1.5rem' }}>
            <Card title="Prerequisites & Outlines">
              <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1' }}>
                Required Baseline: **English Placement score &gt;= 60** or course **General Syntax Grammar 1**.
              </p>
            </Card>
        ...
        {activeTab === 'CURRICULUM' && (
          <Card title="Sequenced Modules tree">
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1' }}>
              Includes 3 Core modules: Syntax Modifiers, Active Vocabulary diagnostics, Paragraph completions. Navigate to Curriculum builder for sequence drag audits.
            </p>
          </Card>
        )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Create, sequence, and version program syllabus guidelines</p>
      </div>
