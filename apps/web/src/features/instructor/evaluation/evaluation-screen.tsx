...
import { instructorAssessmentService, SubmissionItem } from '../../../services/instructor/assessment.service';
...
export function EvaluationScreen() {
  ...
  async function handleOverride(e: React.FormEvent) {
    ...
    const success = await instructorAssessmentService.overrideScore(selectedSub.id, scoreNum, commentText);
    if (success) {
      setNotification(`Score overridden to ${scoreNum} for ${selectedSub.studentName}!`);
      setTimeout(() => setNotification(null), 3000);
      setSubmissions(prev =>
        prev.map(s => (s.id === selectedSub.id ? { ...s, aiScore: scoreNum, evaluated: true } : s))
      );
      setSelectedSub(null);
      setOverrideVal('');
      setCommentText('');
  ...
  const columns = [
    ...
    },
    {
      header: 'Actions',
      render: (row: SubmissionItem) => (
        <Button onClick={() => { setSelectedSub(row); setOverrideVal(String(row.aiScore)); }}>
          Calibrate Score
        </Button>
      )
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Review grammar grading rubrics, essays, and apply overrides</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selectedSub ? '1fr 340px' : '1fr', gap: '2rem', alignItems: 'start' }}>
        <BulkTable
          data={submissions}
          columns={columns}
        />
        ...
        {selectedSub && (
          <Card title="Score Calibration Override">
            <form onSubmit={handleOverride} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              />
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.5rem' }}>Comments / Justification</label>
                <textarea
                  value={commentText}
              ...
              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                <Button variant="ghost" type="button" onClick={() => setSelectedSub(null)}>Cancel</Button>
