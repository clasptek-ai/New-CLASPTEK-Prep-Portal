...
import {
  instructorMockResultsService,
  MockExamResult,
} from '../../../services/instructor/mock-results.service';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800 }}>
            Mock Examinations Results
          </h1>
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>
            Analyze section-wise test performance scoreboards and trigger PDF report summaries
          </p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        {mockResults.map((mock) => (
          <Card
            key={mock.id}
            title={`${mock.name} — ${mock.studentName}`}
            actions={<Button onClick={() => handleExport(mock.name)}>Export PDF Report</Button>}
          >
            <div
              style={{
                display: 'flex',
            ...
            >
              ...
              >
                <div>
                  Overall Score: <strong>{mock.score}%</strong>
                </div>
                <div>
                  Percentile: <strong>{mock.percentile}th</strong>
                </div>
                <div>
                  Time Used: <strong>{mock.timeUsed}</strong>
                </div>
                <div>
                  Incorrect count: <strong>{mock.incorrectQuestions.length}</strong>
                </div>
              </div>
              ...
              <div>
                ...
                >
                  <span>
                    Listening: <strong>{mock.sectionScores.listening}</strong>
                  </span>
                  <span>
                    Reading: <strong>{mock.sectionScores.reading}</strong>
                  </span>
                  <span>
                    Writing: <strong>{mock.sectionScores.writing}</strong>
                  </span>
                  <span>
                    Speaking: <strong>{mock.sectionScores.speaking}</strong>
                  </span>
                </div>
              </div>
