...
import {
  ...
  RiskBadge,
  Timeline,
} from '../../../components/instructor/instructor-components';
import {
  instructorStudentsService,
  StudentProfile,
} from '../../../services/instructor/students.service';
import {
...
export function StudentsScreen({ studentId }: { studentId?: string }) {
  ...
  const [activeTab, setActiveTab] = useState<
    'OVERVIEW' | 'JOURNEY' | 'PRACTICE' | 'ASSIGNMENTS' | 'MOCKS' | 'READINESS' | 'INSTRUCTOR_NOTES'
  >('OVERVIEW');
    ...
    return (
      <Card style={{ borderLeft: '4px solid #ef4444' }}>
        ...
        <p style={{ margin: '0.5rem 0 0 0', color: '#cbd5e1', fontSize: '0.85rem' }}>
          Strict Scoping Rules: You are not authorized to view students outside your assigned
          programmes.
        </p>
        <Button style={{ marginTop: '1rem' }} onClick={() => router.push('/instructor/students')}>
          Return to Directory
        </Button>
      </Card>
    );
  ...
  const columns = [
    {
      ...
      render: (row: StudentProfile) => (
        <span
          ...
          onClick={() => router.push(`/instructor/students/${row.id}`)}
        >
          {row.name}
        </span>
      ),
    ...
    },
    {
      ...
      render: (row: StudentProfile) => (
        <span style={{ fontWeight: 700 }}>{row.readinessScore}%</span>
      ),
    },
    {
      header: 'Risk Level',
  ...
  if (selectedStudent) {
    const studentEvents = [
      {
        date: '2026-07-16',
        title: 'Submitted Exam Diagnostic Test A',
        desc: 'Auto-evaluation completed with band prediction of 7.0',
        category: 'Exam',
      },
      {
        date: '2026-07-12',
        title: 'Flagged as At-Risk by Prediction Engine',
        desc: 'Readiness score dipped below target parameters due to inactive study gaps',
        category: 'Alert',
      },
      {
        date: '2026-07-10',
        title: 'Activated Vocabulary Practice exercises',
        desc: 'Completed 15 vocabulary blocks',
        category: 'Practice',
      },
    ];
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        {/* Profile Header */}
        <Card
          title={`Student Workspace: ${selectedStudent.name}`}
          actions={<RiskBadge status={selectedStudent.riskStatus} />}
        >
          <div
            style={{
              display: 'grid',
          ...
          >
            ...
            <div>
              ...
              <strong style={{ display: 'block', color: '#f8fafc', marginTop: '0.25rem' }}>
                {selectedStudent.milestoneCompletion}%
              </strong>
            </div>
          </div>
        </Card>
          ...
          >
            ...
            <Card title="AI Advisor Note">
              <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1', fontStyle: 'italic' }}>
                &ldquo;Grammar active modifiers accuracy has improved by 12% following practice
                sessions. Recommend setting up essay assignments.&rdquo;
              </p>
            </Card>
          </div>
        )}
        ...
        {activeTab === 'PRACTICE' && (
          <Card title="Practice Analytics">
            ...
            >
              <div>
                Practice Accuracy: <strong>72.5%</strong>
              </div>
              <div>
                Time spent: <strong>4.2 hours</strong>
              </div>
              <div>
                Streak: <strong>5 days active</strong>
              </div>
            </div>
          </Card>
        )}
        ...
        {activeTab === 'ASSIGNMENTS' && (
          <Card title="Assignments & Exercises Status">
            ...
            >
              <div>
                Advanced Essay Syntax: <Badge variant="success">GRADED (85/100)</Badge>
              </div>
              <div>
                IELTS Vocabulary exercises: <Badge variant="warning">PENDING SUBMISSION</Badge>
              </div>
            </div>
          </Card>
        )}
          ...
          <Card title="Diagnostic Mocks History">
            ...
            >
              <div style={{ padding: '0.75rem', backgroundColor: '#0b0f19', borderRadius: '6px' }}>
                <div>
                  Mock Name: <strong>Diagnostic Test A</strong>
                </div>
                <div>
                  Score: <strong>82%</strong> | Date: 2026-07-16
                </div>
                <div>
                  Incorrect Questions: <strong>#12, #24, #31</strong>
                </div>
              </div>
            </div>
          </Card>
        ...
        {activeTab === 'READINESS' && readiness && (
          <Card title="Prediction Readiness Details">
            ...
            >
              <div>
                Overall Readiness Score: <strong>{readiness.overallReadiness}%</strong>
              </div>
              <div>
                Confidence Level: <strong>{readiness.confidence}%</strong>
              </div>
              <div>
                Risk Tier:{' '}
              ...
              <div>
                Priority Study Plan: <em>{readiness.recommendedStudyPlan}</em>
              </div>
              <div>
                Suggested mock exam date: <strong>{readiness.suggestedMockDate}</strong>
              </div>
            </div>
          </Card>
        )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>
            Search, select, and manage student learning plans
          </p>
        </div>
      ...
      <BulkTable
        ...
        columns={columns}
        onBulkAction={(items) =>
          showBanner(`Bulk Messaging triggered for ${items.length} students`)
        }
