  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800 }}>Instructor Profile & Settings</h1>
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Update security credentials and manage alerts parameters</p>
        </div>
        ...
        <Card title="Change Security Password">
          <form onSubmit={handlePasswordChange} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '400px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8', marginBottom: '0.25rem' }}>Old Password</label>
              <input
                type="password"
                value={oldPassword}
        ...
        <Card title="Active Login Sessions">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {sessionLogs.map((log, i) => (
              <div key={i} style={{ padding: '0.75rem', borderRadius: '8px', backgroundColor: '#0b0f19', borderLeft: '3px solid #f59e0b', fontSize: '0.8rem', color: '#cbd5e1' }}>
                <div>Device: <strong>{log.device}</strong></div>
                <div>IP: {log.ip}</div>
                <div style={{ color: '#64748b', fontSize: '0.7rem', marginTop: '0.25rem' }}>Logged at {log.time}</div>
              </div>
            ))}
          </div>
        </Card>
