...
export function CohortsScreen({ cohortId }: { cohortId?: string }) {
  ...
  const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'STUDENTS' | 'ATTENDANCE' | 'COMPETENCY'>('OVERVIEW');
  ...
  const columns = [
    {
      ...
      render: (row: CohortItem) => (
        <span style={{ fontWeight: 600, cursor: 'pointer', color: '#60a5fa' }} onClick={() => router.push(`/instructor/cohorts/${row.id}`)}>
          {row.name}
        </span>
      )
    },
    ...
    },
    {
      ...
      render: (row: CohortItem) => (
        <Badge variant={row.atRiskCount > 5 ? 'danger' : row.atRiskCount > 1 ? 'warning' : 'success'}>
    ...
    },
    {
      header: 'Actions',
      render: (row: CohortItem) => (
        <Button onClick={() => router.push(`/instructor/cohorts/${row.id}`)}>
          Manage
        </Button>
      )
    }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <Card title={`Cohort Workspace: ${selectedCohort.name}`}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', fontSize: '0.9rem', color: '#cbd5e1' }}>
            <div>
              ...
              <strong style={{ display: 'block', color: '#f8fafc', marginTop: '0.25rem' }}>{selectedCohort.enrolledStudentsCount}</strong>
            </div>
            <div>
              <span>Readiness Average:</span>
              <strong style={{ display: 'block', color: '#f8fafc', marginTop: '0.25rem', fontSize: '1.25rem' }}>{selectedCohort.averageReadiness}%</strong>
            </div>
            <div>
              <span>Syllabus Completion:</span>
              <strong style={{ display: 'block', color: '#f8fafc', marginTop: '0.25rem' }}>{selectedCohort.completionRate}%</strong>
            </div>
          </div>
        </Card>

        {/* Tab layout */}
        <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid #232e48', paddingBottom: '0.5rem' }}>
        ...
        {activeTab === 'STUDENTS' && (
          <Card title="Cohort Student List">
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1' }}>
              Roster includes John Doe, Jane Smith, Bob Johnson. Navigate to the Student Directory for full portfolio detail reviews.
            </p>
          </Card>
        )}
        ...
        {activeTab === 'ATTENDANCE' && (
          <Card title="Attendance Summary Logs">
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1' }}>
              Diagnostic Mock Exams Attendance is at **98%** (24 out of 25 students completed session B).
            </p>
          </Card>
        )}
        ...
        {activeTab === 'COMPETENCY' && (
          <Card title="Competencies Heatmap Analysis">
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1' }}>
              Advanced Vocabulary shows standard high variance risk (avg 52%). Modifiers and active syntax are in high success limits (avg 81%).
            </p>
          </Card>
        )}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Monitor classroom learning aggregates, milestones progress, and rankings</p>
      </div>
