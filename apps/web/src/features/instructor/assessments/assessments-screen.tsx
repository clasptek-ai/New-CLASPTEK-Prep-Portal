  ...
  const columns = [
    {
      ...
      render: (row: typeof library[0]) => (
        <span style={{ fontWeight: 600, cursor: 'pointer', color: '#60a5fa' }} onClick={() => setSelectedId(row.id)}>
          {row.title}
        </span>
      )
    },
    ...
    },
    {
      header: 'Status',
      render: (row: typeof library[0]) => (
        <Badge variant={row.status === 'PUBLISHED' ? 'success' : 'info'}>
          {row.status}
        </Badge>
      )
    },
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800 }}>Assessment Monitor Workspace</h2>
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Live session completions and timer status values</p>
          </div>
          <Button variant="secondary" onClick={() => setSelectedId(null)}>Back to Library</Button>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
          <Card title="Live Submissions Queue">
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#cbd5e1' }}>
              No live active timers in this session. All 25 students submitted. Auto AI score calculation outputs are ready.
            </p>
          </Card>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Create schedules and inspect mock diagnostic records</p>
        </div>
      ...
      {showSchedule && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(11, 15, 25, 0.7)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ maxWidth: '500px', width: '100%', backgroundColor: '#151d30', border: '1px solid #232e48', borderRadius: '12px', padding: '1.5rem' }}>
            ...
            <form onSubmit={handleScheduleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <Input label="Mock Exam Title" value={title} onChange={(e) => setTitle(e.target.value)} required />
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#94a3b8', marginBottom: '0.5rem' }}>Select Cohort</label>
                <select
                  value={cohort}
              ...
              <Input label="Release Date & Time" type="datetime-local" value={date} onChange={(e) => setDate(e.target.value)} required />
