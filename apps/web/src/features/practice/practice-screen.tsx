...
export function AdaptivePracticeScreen() {
  ...
  const PRACTICE_MODES = [
    { id: 'ADAPTIVE', label: 'Adaptive Practice', sub: 'Dynamic AI question sizing', icon: <Zap size={18} color="#38bdf8" /> },
    { id: 'SKILL', label: 'Skill Building', sub: 'Target specific grammar & reading skills', icon: <Target size={18} color="#34d399" /> },
    { id: 'TOPIC', label: 'Topic Focus', sub: 'Drill questions by topic category', icon: <BookOpen size={18} color="#60a5fa" /> },
    { id: 'TIMED', label: 'Timed Simulation', sub: 'Exam time constraint practice', icon: <Clock size={18} color="#fbbf24" /> },
    { id: 'WEAK_SKILL', label: 'Weakness Booster', sub: 'Focus on lowest accuracy topics', icon: <Flame size={18} color="#f87171" /> },
    { id: 'EXAM_BOOSTER', label: 'Exam Booster', sub: 'High-yield examination items', icon: <Award size={18} color="#a78bfa" /> },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: '100%', boxSizing: 'border-box' }}>
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.9rem', color: '#cbd5e1', maxWidth: '640px' }}>
            AI-driven skill building, real-time diagnostic evaluation, spaced retention tracking, and personalized target management.
          </p>
        </div>

        {stats && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
            <div style={{ padding: '0.65rem 1rem', borderRadius: '12px', backgroundColor: 'rgba(52, 211, 153, 0.1)', border: '1px solid rgba(52, 211, 153, 0.25)', textAlign: 'right' }}>
              <div style={{ fontSize: '0.7rem', fontWeight: 700, color: '#34d399', textTransform: 'uppercase' }}>Overall Accuracy</div>
              <div style={{ fontSize: '1.2rem', fontWeight: 800, color: '#ffffff', marginTop: '2px' }}>{stats.accuracy}%</div>
            </div>
          </div>
        )}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <Card style={{ padding: '1.75rem', borderRadius: '16px', backgroundColor: '#151d30', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
              <div style={{ fontSize: '1.05rem', fontWeight: 800, color: '#f8fafc', marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
                {PRACTICE_MODES.map((mode) => (
                  ...
                  >
                    ...
                    <div>
                      <div style={{ fontSize: '0.95rem', fontWeight: 700, color: '#ffffff' }}>{mode.label}</div>
                      <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '2px' }}>{mode.sub}</div>
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div>
            <Card style={{ padding: '1.5rem', borderRadius: '16px', backgroundColor: '#151d30', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', fontSize: '0.85rem', color: '#cbd5e1' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.6rem 0.85rem', backgroundColor: '#0f172a', borderRadius: '8px' }}>
                ...
                <div>
                  <span style={{ display: 'block', fontWeight: 700, color: '#34d399', marginBottom: '0.4rem', fontSize: '0.8rem', textTransform: 'uppercase' }}>
                ...
                <div>
                  <span style={{ display: 'block', fontWeight: 700, color: '#f87171', marginBottom: '0.4rem', fontSize: '0.8rem', textTransform: 'uppercase' }}>
                    Weakest Focus Areas
                  </span>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
                    {stats.weakTopics.map((t, idx) => (
                      <Badge key={idx} variant="danger">{t}</Badge>
                    ))}
                  </div>
                </div>
              </div>
      ...
      {activeQuestion && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <Card style={{ padding: '2rem', borderRadius: '16px', backgroundColor: '#151d30', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <Badge variant="info">Mode: {practiceMode?.replace('_', ' ')}</Badge>
              <Button variant="secondary" onClick={() => setActiveQuestion(null)} style={{ color: '#94a3b8' }}>
                Exit Arena
              </Button>
            </div>

            <div style={{ fontSize: '1.15rem', fontWeight: 700, color: '#ffffff', marginBottom: '1.75rem', lineHeight: 1.45 }}>
              {activeQuestion.text}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              {activeQuestion.options.map((opt, i) => {
                ...
                return (
                  ...
                  >
                    <span>{opt}</span>
                    {selectedOption && isCorrectOption && <span style={{ fontSize: '0.8rem', fontWeight: 800, color: '#34d399' }}>✓ Correct</span>}
                    {selectedOption && isSelected && !isCorrectOption && <span style={{ fontSize: '0.8rem', fontWeight: 800, color: '#f87171' }}>✗ Selected</span>}
                  </button>
                );
              })}
            </div>
          ...
          {selectedOption && (
            <Card style={{ padding: '1.5rem', borderRadius: '14px', backgroundColor: '#151d30', border: '1px solid rgba(59, 130, 246, 0.25)' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div style={{ fontSize: '0.9rem', fontWeight: 800, color: '#60a5fa' }}>Explanation & Learning Rationale</div>
                <p style={{ margin: 0, fontSize: '0.875rem', color: '#cbd5e1', lineHeight: 1.5 }}>{explanation}</p>
                <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                  <Button variant="primary" onClick={handleNextQuestion} style={{ backgroundColor: '#2563eb', color: '#ffffff', gap: '0.4rem', display: 'flex', alignItems: 'center' }}>
