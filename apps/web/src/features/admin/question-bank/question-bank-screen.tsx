...
export function QuestionBankScreen() {
  ...
  async function handleReject(id: string) {
    const success = await adminQuestionsService.rejectQuestion(id, 'Fails curriculum modifiers specifications.');
    if (success) {
      setQuestions(prev => prev.filter(q => q.id !== id));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: '100%' }}>
      <div>
        <h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800 }}>Question Bank Approval Hub</h1>
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Audit imported questions, verify answers structures, and assign difficulty objectives</p>
      </div>
      ...
      <Table
        data={questions}
        columns={[
          { header: 'Question Prompt', render: row => <span style={{ fontWeight: 500 }}>{row.text}</span> },
          { header: 'Type', render: row => <Badge>{row.type}</Badge> },
          { header: 'Difficulty', render: row => <Badge variant={row.difficulty === 'HARD' ? 'danger' : row.difficulty === 'MEDIUM' ? 'warning' : 'success'}>{row.difficulty}</Badge> },
          ...
          { header: 'Status', render: row => <Badge variant="warning">{row.status}</Badge> },
          {
            ...
            render: row => (
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <Button variant="secondary" onClick={() => handleReject(row.id)}>Reject</Button>
              </div>
            )
          }
        ]}
