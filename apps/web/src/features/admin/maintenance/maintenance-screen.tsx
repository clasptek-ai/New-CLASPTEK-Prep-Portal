    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>
          Schedule downtime windows, run manual database backups, and clean caches
        </p>
      </div>
      ...
      >
        ...
        <Card title="System Tools & Controls">
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <Button
              variant="secondary"
              onClick={() => showInfo('Triggering database snapshot backup...')}
            >
              Backup database
            </Button>
            <Button variant="secondary" onClick={() => showInfo('Clearing redis memory cache...')}>
              Clear cache
            </Button>
          </div>
        </Card>
