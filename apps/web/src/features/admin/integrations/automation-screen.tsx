...
export function AutomationScreen() {
  const [workflows, setWorkflows] = useState([
    {
      id: '1',
      trigger: 'Student Mock Exam Completed',
      condition: 'Score >= 80%',
      action: 'Unlock Practice Lesson 4',
      status: 'ACTIVE',
    },
    {
      id: '2',
      ...
      action: 'Lock Organization Tenant Access',
      status: 'ACTIVE',
    },
  ]);
  ...
  const templates = [
    {
      name: 'Student Registration Preset',
      description: 'Triggers on student registration, sends welcome email and sets up study plan.',
    },
    {
      name: 'Assessment Completion Alerts',
      description: 'Triggers on assessment submission, pushes notification log to instructor.',
    },
    {
      name: 'Certificate Issuance Workflow',
      description: 'Triggers on program completion, fires automated credential badge.',
    },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>
            Define event triggers, conditional logic pipelines, and system execution actions
          </p>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
        {workflows.map((flow) => (
          <Card
            key={flow.id}
            title={`Workflow #${flow.id}`}
            actions={<Badge variant="success">{flow.status}</Badge>}
          >
            <div
              style={{
                display: 'flex',
      ...
      >
        {templates.map((tpl, i) => (
          <Card key={i} title={tpl.name}>
            ...
            <Button variant="secondary" onClick={() => handleAddTemplate(tpl)}>
              Use Template
            </Button>
          </Card>
        ))}
