...
import { Card, Button, Badge, SkeletonLoader } from '../../../components/ui/ui-components';
...
export function AdminDashboardScreen() {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: '100%' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
          <SkeletonLoader height="240px" borderRadius="12px" />
          <SkeletonLoader height="240px" borderRadius="12px" />
        </div>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', width: '100%' }}>
      ...
      <Card
        style={{
          background: 'linear-gradient(135deg, #ec489920, #0b0f19)',
        ...
        >
          <div>
            ...
            <p style={{ margin: '0.5rem 0 0 0', color: '#cbd5e1' }}>
              Active Term: <strong>{academicTerm}</strong> | System Health:{' '}
              <Badge variant={systemHealth === 'HEALTHY' ? 'success' : 'danger'}>
                {systemHealth}
              </Badge>
            </p>
          </div>
          ...
          >
            <Card title="Total Platform Users">
              ...
              <div style={{ fontSize: '0.8rem', color: '#64748b' }}>
                Students: {stats.activeStudents} | Instructors: {stats.activeInstructors}
              </div>
            </Card>
            ...
            <Card title="Pending Approvals">
              <div
                style={{ fontSize: '2rem', fontWeight: 800, marginTop: '0.5rem', color: '#f59e0b' }}
              >
                {pendingApprovals}
              </div>
              <div style={{ fontSize: '0.8rem', color: '#64748b' }}>
                Requires validation actions
              </div>
            </Card>
          </div>
            ...
            <Column span={6}>
              <Card title="Quick Action Operations">
                ...
                >
                  ...
                  <Button variant="ghost" onClick={() => router.push('/admin/curriculum')}>
                    Publish Curriculum
                  </Button>
                  <Button variant="ghost" onClick={() => router.push('/admin/question-bank')}>
                    Review Questions
                  </Button>
                  <Button variant="secondary" onClick={() => router.push('/admin/resources')}>
                    Upload Resource
                  </Button>
                </div>
              </Card>
                  ...
                  {data.notifications.map((n) => (
                    <div
                      ...
                      style={{
                        ...
                        borderLeft: `3px solid ${n.severity === 'CRITICAL' ? '#ef4444' : '#f59e0b'}`,
                      }}
                    >
                      <span
                        style={{
              ...
              {data.recentActivity.map((act) => (
                ...
                >
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#64748b' }}>
                    By: {act.user} | {new Date(act.timestamp).toLocaleTimeString()}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              >
                <p style={{ margin: '0 0 0.5rem 0' }}>
                  ✔ Supabase RLS Enforcement check: **Enforced**
                </p>
