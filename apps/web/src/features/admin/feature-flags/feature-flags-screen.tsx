...
export function FeatureFlagsScreen() {
  ...
  const [flags, setFlags] = useState([
    ...
    { id: '3', name: 'experimentalDashboards', category: 'Analytics', status: 'ENABLED', rollout: '50%' }
  ]);

  const handleToggle = (id: string) => {
    setFlags(prev =>
      prev.map(f => (f.id === id ? { ...f, status: f.status === 'ENABLED' ? 'DISABLED' : 'ENABLED', rollout: f.status === 'ENABLED' ? '0%' : '100%' } : f))
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div>
        ...
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Staged rollouts, experimental AI switches, and beta release configurations</p>
      </div>
          ...
          <Card key={flag.id} title={flag.name} actions={<Badge>{flag.category}</Badge>}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: '0.85rem', color: '#cbd5e1' }}>
                ...
                <p style={{ margin: 0 }}>Status: <strong style={{ color: flag.status === 'ENABLED' ? '#10b981' : '#ef4444' }}>{flag.status}</strong></p>
              </div>
