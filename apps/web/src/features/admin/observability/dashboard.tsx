...
import { MetricCard, ServiceDependencyGraph, AlertCard } from '../../../components/admin/observability/observability-components';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#94a3b8' }}>Monitor latency spikes, alert levels, database pools, and request traces</p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <Button onClick={() => router.push('/admin/observability/metrics')}>Metrics Explorer</Button>
          <Button variant="secondary" onClick={() => router.push('/admin/observability/traces')}>Traces Timeline</Button>
          <Button variant="secondary" onClick={() => router.push('/admin/observability/alerts')}>Alert Center</Button>
        </div>
      </div>
        ...
        <Column span={6}>
          <Card title="Operational Alerts Summary">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <AlertCard severity="WARNING" category="INFRA" message="Database connection count exceeds average by 10%" timestamp="10m ago" />
              <AlertCard severity="INFO" category="DEPLOY" message="Staging deployment verification successful" timestamp="1h ago" />
            </div>
          </Card>
        </Column>
      </Row>
