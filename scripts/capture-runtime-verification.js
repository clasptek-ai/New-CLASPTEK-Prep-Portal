...
async function runRuntimeVerification() {
  ...
  const passageRes = await pool.query(`SELECT id, code, title, content FROM public.reading_passages WHERE status = 'published' OR status IS NOT NULL ORDER BY created_at DESC LIMIT 1`);
  ...
  console.log(`SQL Query Result: SELECT COUNT(*) FROM assessment_attempt_answers WHERE attempt_id = '${attemptId}';`);
  console.log(`Result: ${dbCount}`);

  // STEP 5 — Paper Snapshot Verification (Requirement #3)
  console.log('\n--- 4. PAPER SNAPSHOT VERIFICATION (REQUIREMENT #3) ---');
  const dbSnapshotRes = await pool.query(`SELECT paper_snapshot FROM public.assessment_attempts WHERE id = $1`, [attemptId]);
  const snapshotData = typeof dbSnapshotRes.rows[0].paper_snapshot === 'string' ? JSON.parse(dbSnapshotRes.rows[0].paper_snapshot) : dbSnapshotRes.rows[0].paper_snapshot;
  ...
  historyRes.rows.forEach(r => {
    console.log(`  - Attempt ID: ${r.attempt_id} | Status: ${r.status} | Score: ${r.overall_score}% | CEFR: ${r.cefr_level} | Band: ${r.predicted_band}`);
  });
