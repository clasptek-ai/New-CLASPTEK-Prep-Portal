...
async function main() {
  ...
  console.log(
    `Auditing ${routeFiles.length} API route files for ApiResponse / envelope compliance...\n`
  );
  ...
  console.log(`\nCompliance Summary:`);
  console.log(
    `  Compliant Route Handlers:     ${compliantRoutes} / ${routeFiles.length} (${Math.round((compliantRoutes / routeFiles.length) * 100)}%)`
  );
