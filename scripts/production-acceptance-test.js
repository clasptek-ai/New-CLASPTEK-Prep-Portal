...
async function runPAT() {
  ...
  if (idxRes.rows.length === 0) {
    console.log('  WARNING: NO UNIQUE INDEX on (attempt_id, question_id) — ON CONFLICT will FAIL at runtime!');
    results.push({ check: 'answers_unique_index', status: 'FAIL', detail: 'No unique constraint found on (attempt_id, question_id)' });
  } else {
    ...
    results.push({ check: 'answers_unique_index', status: hasUnique ? 'PASS' : 'FAIL', detail: JSON.stringify(idxRes.rows) });
  }
  ...
  results.push({ check: 'status_constraint', status: constraintRes.rows.length > 0 ? 'PASS' : 'FAIL', detail: JSON.stringify(constraintRes.rows) });
  ...
  results.push({ check: 'reading_passages_no_deleted_at', status: hasDeletedAt ? 'FAIL' : 'PASS', detail: hasDeletedAt ? 'CRITICAL: deleted_at column found!' : 'CLEAR: no deleted_at column' });
  ...
  const originalSnapshot = { snapshotVersion: 1, grammarQuestions: [{ id: 'qtest01', prompt: 'ORIGINAL PROMPT: never changes', options: [] }] };
  ...
  const snapCheck = await pool.query(`SELECT paper_snapshot FROM public.assessment_attempts WHERE id = $1`, [testId]);
  ...
  results.push({ check: 'snapshot_immutability', status: immutable ? 'PASS' : 'FAIL', detail: 'Snapshot read back unchanged after simulated admin edit' });
  ...
  console.log(`  Resume returned existing attempt: ${resumed ? 'PASS' : 'FAIL'} (attemptId: ${activeRes.rows[0]?.id})`);
  results.push({ check: 'resume_no_duplicate', status: resumed ? 'PASS' : 'FAIL', detail: `Active attempt id matches: ${resumed}` });
  ...
  const payload = typeof ans?.response_payload === 'string' ? JSON.parse(ans.response_payload) : ans?.response_payload;
  ...
  results.push({ check: 'autosave_persistence', status: autosaved ? 'PASS' : 'FAIL', detail: `Response payload: ${JSON.stringify(payload)}` });

  await pool.query(`DELETE FROM public.assessment_attempt_answers WHERE attempt_id = $1`, [autosaveAttemptId]);
  ...
  const postSub = await pool.query(`SELECT status, score, closed_at FROM public.assessment_attempts WHERE id = $1`, [subAttemptId]);
  ...
  results.push({ check: 'status_transition_submitted', status: submittedOk ? 'PASS' : 'FAIL', detail: `status=${postSub.rows[0]?.status}, score=${postSub.rows[0]?.score}` });
  ...
  results.push({ check: 'duplicate_submission_prevention', status: noReopen ? 'PASS' : 'FAIL', detail: 'status=SUBMITTED cannot be re-opened by IN_PROGRESS query' });
  ...
  const eventsOk = eventTypes.includes('ATTEMPT_CREATED') && eventTypes.includes('AUTO_SAVE') && eventTypes.includes('SUBMITTED');
  results.push({ check: 'event_logging', status: eventsOk ? 'PASS' : 'FAIL', detail: eventTypes.join(', ') });

  await pool.query(`DELETE FROM public.assessment_attempt_events WHERE attempt_id = $1`, [evAttemptId]);
