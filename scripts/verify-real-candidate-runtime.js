...
async function runRealCandidateRuntimeVerification() {
  ...
  if (!prodCandidate) {
    ...
    const userRes = await pool.query(
      `SELECT id, email, created_at, updated_at, last_sign_in_at, email_confirmed_at FROM auth.users ORDER BY created_at ASC LIMIT 1`
    );
  ...
  const { data: pageSessionData, error: pageSessionErr } =
    await candidateSupabase.auth.getSession();
  ...
  console.log(
    `   updateUser Error:    ${updateRes.error ? JSON.stringify(updateRes.error) : 'null'}`
  );
  ...
  console.log(
    `  - updated_at Changed?     ${updatedRecord.updated_at > initialRecord.updated_at ? 'YES (VERIFIED)' : 'NO'}`
  );
  console.log(
    `  - Password Hash Changed?  ${updatedRecord.encrypted_password !== initialRecord.encrypted_password ? 'YES (VERIFIED)' : 'NO'}`
  );
  ...
  const paperSnapshot = {
    ...
    readingPassage: {
      id: randomUUID(),
      code: 'READ-P01',
      title: 'Production Assessment Passage',
  ...
  console.log(
    `   Loaded ${paperSnapshot.grammarQuestions.length} grammar questions & ${paperSnapshot.readingPassage.comprehensionQuestions.length} reading questions.`
  );
  ...
  const attemptRow = (
    await pool.query('SELECT id, status, score FROM public.assessment_attempts WHERE id = $1', [
      attemptId,
    ])
  ).rows[0];
  const resultRow = (
    await pool.query('SELECT id FROM public.assessment_results WHERE attempt_id = $1', [attemptId])
  ).rows[0];
