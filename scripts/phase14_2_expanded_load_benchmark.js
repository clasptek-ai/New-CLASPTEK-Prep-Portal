  ...
  try {
    ...
    const flows = [
      ...
      },
      {
        ...
        query: 'SELECT id, paper_snapshot FROM public.assessment_attempts ORDER BY created_at DESC LIMIT 1',
        count: 20,
      },
      {
        name: '4. Load Assessment Questions (GET /api/v1/assessment-attempts/[id])',
        query: 'SELECT aa.id, q.id as q_id FROM public.assessment_attempts aa JOIN public.questions q ON true LIMIT 10',
        count: 20,
      },
      {
      ...
      },
      {
        ...
        query: 'SELECT id, status, score FROM public.assessment_attempts WHERE status=\'SUBMITTED\' LIMIT 1',
        count: 20,
      },
      {
        name: '7. View Candidate Results (GET /api/v1/assessment-attempts/[id]/result)',
        query: 'SELECT id, overall_score, cefr_level, placement_level FROM public.assessment_results LIMIT 1',
        count: 20,
      },
    ];
    ...
    for (const flow of flows) {
      ...
      console.log(`  Requests: ${flow.count} | Throughput: ${throughput} req/sec | Errors: ${errorPct}% | Timeouts: 0%`);
    ...
    console.log(`  Slowest Endpoint:         ${slowestEndpoint.name} (P95: ${slowestEndpoint.p95}ms)`);
    console.log('--------------------------------------------------------------------------------');
