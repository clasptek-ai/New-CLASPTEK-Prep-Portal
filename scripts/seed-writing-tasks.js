...
async function main() {
  console.log('=== SEEDING CANONICAL ENGLISH PROFICIENCY WRITING TASKS (LETTER & ESSAY) ===');

  // Insert Task 1 (Letter Writing) if not exists
  await pool.query(`
  ...
  // Insert Task 2 (Essay Writing) if not exists
  await pool.query(`
    INSERT INTO public.writing_tasks (
      id, code, exam_type, task_number, title, prompt, instructions, min_words, time_recommended_minutes, tenant_id
    ) VALUES (
