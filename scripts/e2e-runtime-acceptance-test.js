...
async function runRuntimeAcceptanceTest() {
  ...
  `,
    [
      ...
      JSON.stringify({
        ...
        last_name: 'Candidate',
        programme: 'English Proficiency Core',
      }),
    ]
  );
      ...
      comprehensionQuestions: [
        {
          ...
          prompt:
            'What is the primary factor driving global energy transitions according to the passage?',
          itemType: 'MCQ',
          options: [
    ...
    writingTasks: [
      {
        ...
        prompt:
          'Some people argue that online education is superior to traditional classrooms. To what extent do you agree or disagree?',
        minWords: 150,
        maxWords: 400,
        itemType: 'ESSAY',
        marks: 10,
  ...
  for (let i = 0; i < 30; i++) {
    ...
    `,
      [
        randomUUID(),
        attemptId,
        grammarQuestions[i].id,
        grammarQuestions[i].versionId,
        JSON.stringify({ selectedOptionCode: selectedCode }),
        isCorrect,
      ]
    );
  }
  ...
  `,
    [
      ...
      readingQuestionId,
      readingVersionId,
      JSON.stringify({ selectedOptionCode: 'A' }),
    ]
  );
  ...
  `,
    [
      ...
      writingVersionId,
      JSON.stringify({
        text: 'In recent years, distance education has transformed learning globally. I strongly agree that digital platforms complement traditional education.',
      }),
    ]
  );
  ...
  `,
    [
      ...
      predictedBand,
      JSON.stringify([
        { sectionCode: 'GRAMMAR', sectionName: 'Grammar & Vocabulary', scorePercentage: 80.0 },
      ]),
      JSON.stringify(['Strong grammatical control', 'Consistent reading comprehension']),
      JSON.stringify(['Complex sentence subordination']),
      recommendedCourse,
      JSON.stringify({
        summary: 'Candidate demonstrates solid B2 proficiency with 80% accuracy in core grammar.',
      }),
    ]
  );
  ...
  console.log(
    'API RESPONSE (GET /api/v1/admin/students/' + testStudentId + '/assessment-history):'
  );
  ...
  const attemptRow = (
    await pool.query(`SELECT * FROM public.assessment_attempts WHERE id = $1`, [attemptId])
  ).rows[0];
  const resultRow = (
    await pool.query(`SELECT * FROM public.assessment_results WHERE attempt_id = $1`, [attemptId])
  ).rows[0];
  const answersRows = (
    await pool.query(`SELECT * FROM public.assessment_attempt_answers WHERE attempt_id = $1`, [
      attemptId,
    ])
  ).rows;
  const eventsRows = (
    await pool.query(
      `SELECT * FROM public.assessment_attempt_events WHERE attempt_id = $1 ORDER BY created_at ASC`,
      [attemptId]
    )
  ).rows;
  ...
  const detailResponse = {
    ...
    data: {
      ...
      answers: answersMap,
      paperSnapshot:
        typeof attemptRow.paper_snapshot === 'string'
          ? JSON.parse(attemptRow.paper_snapshot)
          : attemptRow.paper_snapshot,
  ...
  const reQueriedAttempt = (
    await pool.query(`SELECT paper_snapshot FROM public.assessment_attempts WHERE id = $1`, [
      attemptId,
    ])
  ).rows[0];
  const reQueriedSnapshot =
    typeof reQueriedAttempt.paper_snapshot === 'string'
      ? JSON.parse(reQueriedAttempt.paper_snapshot)
      : reQueriedAttempt.paper_snapshot;
  ...
  console.log(
    `   - Immutability Confirmation: Admin Inspector loads exclusively from paper_snapshot and remains immune to live question bank mutations.`
  );
