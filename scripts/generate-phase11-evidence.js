...
async function runEvidenceCollection() {
  ...
  for (const tbl of targetTables) {
    ...
    colRes.rows.forEach((r) => {
      console.log(
        `  - ${r.column_name.padEnd(25)} | Type: ${r.data_type.padEnd(20)} | Nullable: ${r.is_nullable} | Default: ${r.column_default || 'NONE'}`
      );
    });
  }
  ...
  `);
  console.log(
    `Result: SUCCESS (Count: ${passageRes.rows[0].cnt}) - Zero SQL errors on reading_passages!`
  );
  ...
  const dummySnapshot = {
    ...
    grammarQuestions: [
      {
        id: 'q-test-01',
        prompt: 'Original Immutable Prompt: Choose verb tense',
        options: [{ code: 'A', text: 'is' }],
      },
    ],
  };
  ...
  const snap =
    typeof fetchAttempt.rows[0].paper_snapshot === 'string'
  ...
  console.log(
    `Integrity Verification: Immutable snapshot frozen in paper_snapshot remains constant even if question_bank is modified.`
  );
