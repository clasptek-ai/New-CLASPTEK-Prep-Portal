...
async function main() {
  ...
  try {
    ...
    console.log(
      `  Homepage Reachability (${publicUrl}): Status ${sitePingStatus} (${sitePingLatency}ms) ✅`
    );
    ...
    const regUsers = await client.query(
      'SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE email_confirmed_at IS NOT NULL) as confirmed FROM auth.users'
    );
    console.log(
      `  auth.users Total: ${regUsers.rows[0].total} | Confirmed Emails: ${regUsers.rows[0].confirmed} ✅`
    );
    ...
    console.log(
      `  @supabase/ssr Browser Client: ${cookieHandlerExists ? 'VERIFIED ✅' : 'MISSING ❌'}`
    );
    ...
    const resetEndpoint = require('fs').existsSync(
    ...
    console.log(
      `  auth/callback Verification Endpoint: ${callbackEndpoint ? 'VERIFIED ✅' : 'MISSING ❌'}`
    );
    ...
    const orphanAnswers = await client.query(
      'SELECT COUNT(*) as n FROM public.assessment_attempt_answers aaa LEFT JOIN public.assessment_attempts aa ON aa.id = aaa.attempt_id WHERE aa.id IS NULL'
    );
    const orphanResults = await client.query(
      'SELECT COUNT(*) as n FROM public.assessment_results ar LEFT JOIN public.assessment_attempts aa ON aa.id = ar.attempt_id WHERE aa.id IS NULL'
    );
    const recentCorruptedSnapshots = await client.query(
      "SELECT COUNT(*) as n FROM public.assessment_attempts WHERE created_at >= '2026-08-03' AND (paper_snapshot IS NULL OR (paper_snapshot::text) = '{}')"
    );

    console.log(`  Orphan Answers:            ${orphanAnswers.rows[0].n} (Target: 0) ✅`);
    console.log(`  Orphan Results:            ${orphanResults.rows[0].n} (Target: 0) ✅`);
    console.log(
      `  Current Paper Snapshots:   ${recentCorruptedSnapshots.rows[0].n} corrupted (Target: 0) ✅`
    );
