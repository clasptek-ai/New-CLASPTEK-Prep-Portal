...
async function main() {
  ...
  const confirmedUsers = await client.query(
    'SELECT count(*) as n FROM auth.users WHERE email_confirmed_at IS NOT NULL AND deleted_at IS NULL'
  );
  ...
  try {
    const sessionRes = await client.query(
      'SELECT count(*) as n FROM auth.sessions WHERE not_after > NOW()'
    );
  ...
  console.log(
    'Server Client:       createSupabaseServerClient (@supabase/ssr via @clasptek/persistence) ✅'
  );
  console.log('API Route Auth:      getAuthenticatedSession() via Bearer token + SSR cookie ✅');
  console.log(
    'Middleware:          Explicit pass-through for /auth/callback, /reset-password, /login, /register, /forgot-password ✅'
  );
  console.log(
    'Forgot Password:     resetPasswordForEmail() with redirectTo=https://portal.clasptek.org/auth/callback?next=/reset-password ✅'
  );
  ...
  console.log(
    'Mobile Auth:         authFetch() attaches Bearer token for WebKit ITP compatibility ✅'
  );
  console.log(
    'Dev Mock Auth:       NEXT_PUBLIC_DEV_MOCK_AUTH gate present, disabled in production ✅'
  );
  ...
  for (const t of tables) {
    ...
    const keyColsToShow = colsRes.rows.filter((c) =>
      [
        'id',
        'attempt_id',
        'student_id',
    ...
    keyColsToShow.forEach((c) => {
      ...
      console.log(
        '  col: ' + c.column_name + ' (' + c.data_type + ') nullable=' + c.is_nullable + hasDefault
      );
    });
  }
  ...
  if (fks.rows.length > 0) {
    fks.rows.forEach((r) =>
      console.log('  ' + r.table_name + '.' + r.column_name + ' → ' + r.foreign_table)
    );
  } else {
  ...
  const grammarCnt = await client.query(
    'SELECT count(DISTINCT q.id) as n FROM public.questions q JOIN public.question_versions qv ON qv.question_id=q.id WHERE q.deleted_at IS NULL'
  );
  ...
  try {
    const defRes = await client.query(
      'SELECT id, code, title, status, duration_minutes FROM public.assessment_definitions ORDER BY created_at DESC LIMIT 5'
    );
  ...
  console.log(
    'Total Grammar Questions:             ' +
      grammarCnt.rows[0].n +
      (parseInt(grammarCnt.rows[0].n) >= 30 ? ' ✅ (≥30 required)' : ' ❌ (<30 — insufficient)')
  );
  console.log('Total Answer Options:                ' + answerOptCnt.rows[0].n);
  console.log(
    'Reading Passages:                    ' +
      passageCnt.rows[0].n +
      (parseInt(passageCnt.rows[0].n) >= 1 ? ' ✅' : ' ❌')
  );
  console.log(
    'Writing Tasks:                       ' +
      writingCnt.rows[0].n +
      (parseInt(writingCnt.rows[0].n) >= 2 ? ' ✅ (≥2 required)' : ' ❌')
  );
  ...
  const levelDist = await client.query(
    "SELECT COALESCE(qv.proficiency_level, 'NULL') as lvl, count(*) as n FROM public.question_versions qv JOIN public.questions q ON q.id=qv.question_id WHERE q.deleted_at IS NULL GROUP BY qv.proficiency_level ORDER BY n DESC"
  );
  ...
  const recentAttempts = await client.query(
    'SELECT id, student_id, status, score, started_at, closed_at FROM public.assessment_attempts ORDER BY started_at DESC LIMIT 5'
  );
  ...
  recentAttempts.rows.forEach((a) => {
    console.log(
      '  AttemptID: ' +
        a.id.substring(0, 12) +
        '... | StudentID: ' +
        a.student_id.substring(0, 12) +
        '... | Status: ' +
        a.status +
        ' | Score: ' +
        a.score +
        ' | Started: ' +
        (a.started_at ? a.started_at.toISOString() : 'null')
    );
  });

  const recentResults = await client.query(
    'SELECT attempt_id, student_id, overall_score, placement_level, cefr_level, predicted_band FROM public.assessment_results ORDER BY generated_at DESC LIMIT 5'
  );
  console.log('\nRecent Assessment Results:');
  recentResults.rows.forEach((r) => {
    console.log(
      '  AttemptID: ' +
        r.attempt_id.substring(0, 12) +
        '... | Level: ' +
        r.placement_level +
        ' | CEFR: ' +
        r.cefr_level +
  ...
  const recentAnswers = await client.query(
    'SELECT aa.attempt_id, count(*) as n, sum(CASE WHEN is_correct THEN 1 ELSE 0 END) as correct FROM public.assessment_attempt_answers aa GROUP BY aa.attempt_id ORDER BY max(aa.updated_at) DESC LIMIT 5'
  );
  console.log('\nRecent Attempt Answer Submissions:');
  recentAnswers.rows.forEach((a) => {
    console.log(
      '  AttemptID: ' +
        a.attempt_id.substring(0, 12) +
        '... | Answers: ' +
        a.n +
        ' | Correct: ' +
        a.correct
  ...
  Object.entries(envVars).forEach(([k, v]) => {
    ...
    else if (k.includes('KEY') || (k.includes('URL') && !k.includes('PUBLIC_APP')))
      console.log(k + ':  SET (' + v.substring(0, 20) + '...) ✅');
  ...
  console.log('================================================================================');
  console.log(
    'Lighthouse: Not verified (requires browser runtime — run manually at https://pagespeed.web.dev)'
  );
  ...
  const inProgressCount = await client.query(
    "SELECT count(*) as n FROM public.assessment_attempts WHERE status='IN_PROGRESS'"
  );
