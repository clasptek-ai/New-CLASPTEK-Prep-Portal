...
async function main() {
  ...
  console.log(
    `  Resolved Canonical App URL: ${resolvedUrl} ${resolvedUrl.includes('clasptek.org') || resolvedUrl.includes('localhost') ? 'PASSED ✅' : 'FAILED ❌'}`
  );
  ...
  if (passed) {
    console.log(
      '✅ ENVIRONMENT & SECRETS SECURITY AUDIT PASSED: ALL REQUIRED ENVS SET & UNTRACKED'
    );
  } else {
