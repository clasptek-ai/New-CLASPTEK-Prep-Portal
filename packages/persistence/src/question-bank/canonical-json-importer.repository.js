...
exports.CanonicalJsonImporterRepository = void 0;
const crypto_1 = require("crypto");
class CanonicalJsonImporterRepository {
    pool;
    supportedProducts = [
        'English Proficiency',
        'IELTS Academic',
    ...
    validateJsonPayload(payload) {
        ...
        for (let idx = 0; idx < questions.length; idx++) {
            ...
            if (itemType === 'MCQ' || itemType === 'MULTIPLE_RESPONSE') {
                ...
                if (q.correctAnswer) {
                    const optionCodes = options.map((o) => (typeof o === 'string' ? 'A' : o.code));
    ...
    async importJsonBatch(payload, uploadedBy = 'admin-001') {
        ...
        try {
            ...
            await client.query(`INSERT INTO public.question_import_batches
         ...
         VALUES ($1, $2, $3, $4, $5, $6, 'IMPORTING', $7, 0, 0, $8, now())`, [
            ...
            for (const q of questions) {
                ...
                const qvId = (0, crypto_1.randomUUID)();
                ...
                await client.query(`INSERT INTO public.questions (id, code, created_at, import_batch_id)
           VALUES ($1, $2, now(), $3)
           ON CONFLICT (code) DO NOTHING`, [qId, code, batchId]);
                ...
                await client.query(`INSERT INTO public.question_versions
           ...
           VALUES ($1, $2, 1, 'published', $3, $4, now(), $5, $6, $7, $8)`, [
                ...
                if (Array.isArray(q.options)) {
                    for (let optIdx = 0; optIdx < q.options.length; optIdx++) {
                        ...
                        await client.query(`INSERT INTO public.answer_options
               (id, question_version_id, option_code, option_text, is_correct, display_order)
               VALUES (gen_random_uuid(), $1, $2, $3, $4, $5)`, [qvId, optCode, optText, isCorrect, optIdx + 1]);
                    }
                }
                importedCount++;
            }
            // Update import batch status to COMPLETED
            await client.query(`UPDATE public.question_import_batches SET
           status = 'COMPLETED',
           successful_records = $1,
           completed_at = now()
         WHERE id = $2`, [importedCount, batchId]);
    ...
    `;
        ...
        const questionsPayload = await Promise.all(res.rows.map(async (r) => {
            const payload = r.payload || {};
            const optRes = await this.pool.query(`SELECT option_code, option_text, is_correct FROM public.answer_options 
           WHERE question_version_id = $1 ORDER BY display_order ASC`, [r.question_version_id]);
    ...
    async getImportHistory() {
        const res = await this.pool.query(`SELECT * FROM public.question_import_batches ORDER BY created_at DESC LIMIT 50`);
    ...
    async rollbackBatch(batchId) {
        ...
        try {
            ...
            await client.query(`UPDATE public.question_import_batches SET status = 'ROLLED_BACK', rolled_back_at = now() WHERE id = $1`, [batchId]);
            await client.query('COMMIT');
        }
        catch (err) {
            await client.query('ROLLBACK');
...
exports.CanonicalJsonImporterRepository = CanonicalJsonImporterRepository;
//# sourceMappingURL=canonical-json-importer.repository.js.map