...
describe('Learning Analytics Postgres Repository Adapters', () => {
  ...
  test('PostgresAnalyticsDashboardRepository saves and queries configurations', async () => {
    ...
    const studDash = new StudentDashboard({ id: 'dash-1', studentId: 'stud-1', isCustomized: true });
  ...
  test('PostgresReportRepository saves report configuration templates and executions', async () => {
    ...
    const exec = new ReportExecution({ id: 'exec-1', reportDefinitionId: 'def-1', status: 'COMPLETED', executedAt: new Date() });
  ...
  test('PostgresExportRepository saves and fetches exported file status', async () => {
    ...
    const job = new ExportJob({ id: 'job-1', format: 'CSV', status: 'COMPLETED', downloadExpiry: new Date(), generatedBy: 'admin-1' });
  ...
  test('PostgresWidgetRepository maps layout plugins', async () => {
    ...
    const def = new WidgetDefinition({ id: 'widget-1', widgetType: 'READINESS_DIAL', displayName: 'Readiness', defaultConfig: {} });
    await repo.saveDefinition(def);
  ...
  test('PostgresStudentDashboardProjectionRepository saves student projections', async () => {
    ...
    expect(querySqls.some(s => s.includes('INSERT INTO student_analytics_dashboard_projections'))).toBe(true);
  ...
  test('PostgresInstructorDashboardProjectionRepository maps cohort indices views', async () => {
    ...
    await repo.save(proj);
    expect(querySqls.some(s => s.includes('INSERT INTO instructor_dashboard_projections'))).toBe(true);
