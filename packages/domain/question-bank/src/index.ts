...
export class QuestionOwnership extends ValueObject<{
  ...
  get copyrightHolder(): string { return this.props.copyrightHolder; }
  get license(): string { return this.props.license; }
  get source(): string { return this.props.source; }
  get reusePolicy(): string { return this.props.reusePolicy; }
  get expirationDate(): Date | null { return this.props.expirationDate; }
}
...
export class QuestionDependency extends ValueObject<{
  ...
  get parentQuestionId(): string { return this.props.parentQuestionId; }
  get childQuestionId(): string { return this.props.childQuestionId; }
  get dependencyType(): string { return this.props.dependencyType; }
}
...
export class Question extends AggregateRoot<string> {
  ...
  public createVersion(
    ...
    const ver = new QuestionVersion(versionId, versionNo, 'DRAFT', title, payload, digitalSignature);
  ...
  public addAnswerOption(versionNo: SemanticVersion, id: string, code: string, text: string, isCorrect: boolean, displayOrder: number): void {
    const version = this.versions.find(v => v.versionNo.equals(versionNo));
    if (!version) {
      throw new Error(`Version ${versionNo.value} not found`);
    }
  ...
  public setSolution(versionNo: SemanticVersion, id: string, explanation: string, incorrectExplanation: string, hint: string, referenceUrl: string, teachingNote: string): void {
    const version = this.versions.find(v => v.versionNo.equals(versionNo));
    if (!version) {
      throw new Error(`Version ${versionNo.value} not found`);
    }
    version.setSolution(new Solution(id, explanation, incorrectExplanation, hint, referenceUrl, teachingNote));
    this.addDomainEvent(new SolutionUpdated(this.id, versionNo.value));
  }

  public setRubric(versionNo: SemanticVersion, id: string, criteria: string, maxPoints: number, description: string): void {
    const version = this.versions.find(v => v.versionNo.equals(versionNo));
    if (!version) {
      throw new Error(`Version ${versionNo.value} not found`);
    }
...
export class ValidationReport extends Entity<string> {
  constructor(id: string, public readonly validatorName: string, public readonly isValid: boolean, public readonly errors: string[]) {
    super(id);
  }
}

export class ReviewerComment extends Entity<string> {
  constructor(id: string, public readonly reviewerId: string, public readonly role: string, public readonly commentText: string, public readonly timestamp: Date) {
    super(id);
  }
}

export class WorkflowHistory extends Entity<string> {
  constructor(id: string, public readonly stage: string, public readonly actorId: string, public readonly comments: string, public readonly timestamp: Date) {
    super(id);
  }
}
...
export class ReviewRequest extends AggregateRoot<string> {
  ...
  public addValidationReport(reportId: string, validatorName: string, isValid: boolean, errors: string[]): void {
  ...
  public addComment(commentId: string, reviewerId: string, role: string, commentText: string): void {
